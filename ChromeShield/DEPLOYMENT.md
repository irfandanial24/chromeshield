# 🚀 Deploying ChromeShield online (free)
...
When you finish, you'll have a link like `https://chromeshield.streamlit.app`
...
6. Open your project folder on your computer:
   `C:\Users\user\Documents\SEM4\FYP\Project\ChromeShield`
7. Select **all the files and folders inside it** (app.py, cli.py, README.md,
   requirements.txt, the `chromeshield` folder, the `samples` folder, the
...
git init
git add .
git commit -m "ChromeShield initial commit"
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/chromeshield.git
...
4. Fill in the form:
   - **Repository:** `YOUR_USERNAME/chromeshield`
   - **Branch:** `main`
...
```
https://chromeshield.streamlit.app
```
...
- **`ModuleNotFoundError: chromeshield`** → the `chromeshield` folder must be
